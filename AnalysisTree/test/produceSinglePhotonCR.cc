...
namespace LooperHelperFunctions{
  using namespace std;
  using namespace MELAStreamHelpers;
  using namespace OffshellCutflow;

  bool looperRule(BaseTreeLooper*, float const&, SimpleEntry&);

}
bool LooperHelperFunctions::looperRule(BaseTreeLooper* theLooper, float const& extWgt, SimpleEntry& commonEntry){
  // Define handlers
...
#define SCALEFACTOR_HANDLER_DIRECTIVES \
  ...
  const float btag_thr = BtagHelpers::getBtagWP(false);
  ...
  if (hasSimpleHLTMenus && hasHLTMenuProperties){
    MELAerr << "LooperHelperFunctions::looperRule: Defining both simple HLT menus and menus with properties is not allowed. Choose only one!" << endl;
    assert(0);
  }
  ...
  if (
    ...
    MELAerr << "LooperHelperFunctions::looperRule: The trigger type 'SinglePhoton' has to be defined in this looper rule!" << endl;
    assert(0);
  }
...
#define HANDLER_DIRECTIVE(TYPE, NAME) \
  if (!NAME){ \
    MELAerr << "LooperHelperFunctions::looperRule: " << #TYPE << " " << #NAME << " is not registered. Please register and re-run." << endl; \
    assert(0); \
  }
  OBJECT_HANDLER_COMMON_DIRECTIVES;
  if (!isData){
...
#define BRANCH_SCALAR_COMMANDS \
  ...
  BRANCH_COMMAND(float, pfmet_pTmiss_XY_JER_PartMomShifts) \
  BRANCH_COMMAND(float, pfmet_phimiss_XY_JER_PartMomShifts) \
  BRANCH_COMMAND(float, pfmet_pTmiss_XY_JER) \
  BRANCH_COMMAND(float, pfmet_phimiss_XY_JER) \
  BRANCH_COMMAND(float, pfmet_pTmiss_XY_PartMomShifts) \
  BRANCH_COMMAND(float, pfmet_phimiss_XY_PartMomShifts) \
  BRANCH_COMMAND(float, pfmet_pTmiss_JER_PartMomShifts) \
  BRANCH_COMMAND(float, pfmet_phimiss_JER_PartMomShifts) \
  BRANCH_COMMAND(float, pfmet_pTmiss_XY) \
  BRANCH_COMMAND(float, pfmet_phimiss_XY) \
  BRANCH_COMMAND(float, pfmet_pTmiss_JER) \
  BRANCH_COMMAND(float, pfmet_phimiss_JER) \
  BRANCH_COMMAND(float, pfmet_pTmiss_PartMomShifts) \
  BRANCH_COMMAND(float, pfmet_phimiss_PartMomShifts) \
  BRANCH_COMMAND(float, pfmet_pTmiss) \
  BRANCH_COMMAND(float, pfmet_phimiss) \
  BRANCH_COMMAND(float, puppimet_pTmiss_PartMomShifts) \
  BRANCH_COMMAND(float, puppimet_phimiss_PartMomShifts) \
  ...
  BRANCH_COMMAND(unsigned int, event_n_ak4jets) \
  BRANCH_COMMAND(float, ak4jets_HT) \
  BRANCH_COMMAND(float, gamma_pt) \
  BRANCH_COMMAND(float, gamma_eta) \
  BRANCH_COMMAND(float, gamma_phi) \
  BRANCH_COMMAND(float, gamma_mass) \
  BRANCH_COMMAND(bool, gamma_isConversionSafe) \
  BRANCH_COMMAND(bool, gamma_passHGGSelection) \
  BRANCH_COMMAND(bool, gamma_isGap) \
  BRANCH_COMMAND(bool, gamma_isEB) \
  BRANCH_COMMAND(bool, gamma_isEE) \
  BRANCH_COMMAND(bool, gamma_isEBEEGap)
...
#undef BRANCH_COMMAND
  ...
  gamma_pt = theChosenPhoton->pt();
  gamma_eta = theChosenPhoton->eta();
  gamma_phi = theChosenPhoton->phi();
  gamma_mass = theChosenPhoton->m();
  gamma_isConversionSafe = theChosenPhoton->testSelection(PhotonSelectionHelpers::kConversionSafe);
  gamma_passHGGSelection = theChosenPhoton->extras.id_cutBased_HGG_Bits;
  gamma_isGap = theChosenPhoton->isGap();
  gamma_isEBEEGap = theChosenPhoton->isEBEEGap();
  gamma_isEB = theChosenPhoton->isEB();
  gamma_isEE = theChosenPhoton->isEE();
...
#undef PTMISS_FILL_COMMAND
  ...
  for (auto const& jet:ak4jets){
    ...
    if (ParticleSelectionHelpers::isTightJet(jet)){
      ...
      if (jet->getBtagValue()>=btag_thr) n_ak4jets_tight_btagged++;

      ak4jets_HT += jet->pt();
      ak4jets_sump4 += jet->p4();
    }
  }
  if (n_ak4jets_tight_btagged>0) return false;
  event_wgt_SFs *= SF_btagging;

  event_n_ak4jets = ak4jets_tight.size();
...
#undef PTMISS_FILL_COMMAND
      ...
      if (!eventFilter.passCommonSkim() || !eventFilter.passMETFilters(EventFilterHandler::kMETFilters_Tight) || !eventFilter.hasGoodVertex()) continue;
