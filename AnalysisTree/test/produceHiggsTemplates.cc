...
void getProcessCollection_ZZ2L2Nu(TString const& strSampleSet, std::vector<TString>& snames){
  if (strSampleSet=="ggZZ_offshell") snames.push_back("GGH_ZZTo2L2Nu_POWHEG");
    ...
    snames.push_back("ZH_HTo4Q_2LFilter_POWHEG");
  ...
  for (unsigned int icat=0; icat<nCats; icat++){
    ...
    case 1:
    {
      ...
      if (!hRaw.empty()){
        ...
        for (auto& hh:hRaw){
          MELAout << "\t\t- Calculating Neff_raw and integral for histogram " << itpl << ":" << endl;
          ...
          if ((doOffshell || itpl>0) && (Neff_raw_minNeff[0]<0. || Neff_raw_minNeff[0]>Neff_raw)){
