#ifndef PRODUCEHIGGSESTIMATES_ZWTO3L1NU_H
#define PRODUCEHIGGSESTIMATES_ZWTO3L1NU_H
...
void produceHiggsEstimates_ZWTo3L1Nu(
...
#define _JETMETARGS_ applyPUIdToAK4Jets, applyTightLeptonVetoIdToAK4Jets, use_MET_Puppi, use_MET_XYCorr, use_MET_JERCorr, use_MET_ParticleMomCorr, use_MET_p4Preservation, use_MET_corrections
  ...
  ACHypothesisHelpers::DecayType dktype = ACHypothesisHelpers::kZW3l1nu;
  ...
  TString const coutput_main = "output/SignalEstimates_ZWTo3L1Nu/" + strdate + "/FinalTrees/" + period;
  ...
  BaseTree* tout = new BaseTree("FinalTree");
  {
    ...
    if (theGlobalSyst){
      ...
      tout->putBranch<float>("dPhi_pTleptonsjets_pTmiss", TMath::Pi());
    }
      ...
      *ptr_event_wgt_SFs_PUJetId = std::min(*ptr_event_wgt_SFs_PUJetId, 3.f);
      ...
      // Record the event to the output trees
      {
        ...
        if (theGlobalSyst){
          ...
          tout->setVal<float>("dPhi_pTleptonsjets_pTmiss", dPhi_pTleptonsjets_pTmiss);
        }
