...
void createGammaTrees(TString strSampleSet, TString period, SystematicsHelpers::SystematicVariationTypes theGlobalSyst = SystematicsHelpers::sNominal){
  ...
  for (auto const& strSample:sampleList){
    ...
    std::vector<SystematicsHelpers::SystematicVariationTypes> allowedSysts; allowedSysts.push_back(SystematicsHelpers::sNominal);
    if (!isData){
    ...
    float xsec=1;
    std::unordered_map<SystematicsHelpers::SystematicVariationTypes, float> sum_wgts;
    for (auto const& syst:allowedSysts) sum_wgts[syst] = (isData ? 1 : 0);
    ...
    if (!isData){
      ...
      for (int ev=0; ev<nEntries; ev++){
        ...
        for (auto const& syst:allowedSysts){
          simEventHandler.constructSimEvent(syst);
    ...
    std::unordered_map<SystematicsHelpers::SystematicVariationTypes, BaseTree*> syst_outtree_map;
    std::unordered_map<SystematicsHelpers::SystematicVariationTypes, bool> firstEvent;
    for (auto const& syst:allowedSysts){
      TString outtreename = "AnalysisTree";
      if (syst!=SystematicsHelpers::sNominal) outtreename = outtreename + "_" + SystematicsHelpers::getSystName(syst);
      BaseTree* outtree = new BaseTree(outtreename);
      outtree->setAutoSave(0);
      syst_outtree_map[syst] = outtree;
      firstEvent[syst] = true;
    }
    ...
    for (int ev=0; ev<nEntries; ev++){
      ...
      for (auto const& syst:allowedSysts){
        BaseTree*& outtree = syst_outtree_map[syst];
        ...
        if (!isData){
          ...
          genInfoHandler.constructGenInfo(syst);
          auto const& genInfo = genInfoHandler.getGenInfo();
        ...
        wgt = genwgt * puwgt * trigwgt * xsec * (isData ? 1.f : lumi) / sum_wgts[syst];
        ...
        jetHandler.constructJetMET(syst, &muons, &electrons, &photons);
        ...
        commonEntry.setNamedVal("genwgt", genwgt / sum_wgts[syst]);
...
#undef DOUBLEVECTOR_DATA_OUTPUT_DIRECTIVE
        ...
        outtree->fill();
        firstEvent[syst] = false;
        if (syst==SystematicsHelpers::sNominal) n_acc++;
      }
    }
