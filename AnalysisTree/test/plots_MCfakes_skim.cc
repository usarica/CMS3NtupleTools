...
void getTrees(int procsel, int do_ee_mumu, TString strdate){
  constexpr bool doOldSelection = true;

  if (do_ee_mumu<0 || do_ee_mumu>1) return;
  if (procsel<0) return;
  ...
  TString const coutput_main = "output/LepEffFromMC/SkimTrees/" + strdate + (do_ee_mumu==0 ? "/Zee" : "/Zmumu");
  ...
  std::vector<std::string> triggerCheckList = do_ee_mumu==0 ?
  ...
  sampleList.emplace_back("QCD_HT_100-200", "QCD (H_{T}-binned)", "QCD_HT_100-200", -1, HistogramProperties((int) kCyan, 1, 2));
  sampleList.emplace_back("QCD_HT_200-300", "QCD (H_{T}-binned)", "QCD_HT_200-300", -1, HistogramProperties((int) kCyan, 1, 2));
  sampleList.emplace_back("QCD_HT_300-500", "QCD (H_{T}-binned)", "QCD_HT_300-500", -1, HistogramProperties((int) kCyan, 1, 2));
  sampleList.emplace_back("QCD_HT_500-700", "QCD (H_{T}-binned)", "QCD_HT_500-700", -1, HistogramProperties((int) kCyan, 1, 2));
  sampleList.emplace_back("QCD_HT_700-1000", "QCD (H_{T}-binned)", "QCD_HT_700-1000", -1, HistogramProperties((int) kCyan, 1, 2));
  sampleList.emplace_back("QCD_HT_1000-1500", "QCD (H_{T}-binned)", "QCD_HT_1000-1500", -1, HistogramProperties((int) kCyan, 1, 2));
  sampleList.emplace_back("QCD_HT_1500-2000", "QCD (H_{T}-binned)", "QCD_HT_1500-2000", -1, HistogramProperties((int) kCyan, 1, 2));
  sampleList.emplace_back("QCD_HT_2000-inf", "QCD (H_{T}-binned)", "QCD_HT_2000-inf", -1, HistogramProperties((int) kCyan, 1, 2));
  ...
  sampleList.emplace_back("ZZ2L2Q", "ZZ#rightarrow2l2q", "qqZZ_2l2q", -1, HistogramProperties((int) kYellow-3, 1, 2));
  sampleList.emplace_back("WW2L2Nu", "WW#rightarrow2l2#nu", "qqWW_2l2nu", -1, HistogramProperties((int) kTeal-1, 1, 2));
  sampleList.emplace_back("WWLNuQQ", "WW#rightarrowl#nuqq", "qqWW_lnu2q", -1, HistogramProperties((int) kTeal-1, 1, 2));
  sampleList.emplace_back("WZ3LNu", "WZ#rightarrow3l#nu", "qqWZ_3lnu_MG", -1, HistogramProperties((int) kTeal-1, 1, 2));
  sampleList.emplace_back("WZ2L2Q", "WZ#rightarrow2l2q", "qqWZ_2l2q", -1, HistogramProperties((int) kTeal-1, 1, 2));
  sampleList.emplace_back("WZL3Nu", "WZ#rightarrowl3#nu", "qqWZ_13nu", -1, HistogramProperties((int) kTeal-1, 1, 2));
  sampleList.emplace_back("WZLNuQQ", "WZ#rightarrowl#nuqq", "qqWZ_lnu2q", -1, HistogramProperties((int) kTeal-1, 1, 2));
...
#undef BRANCH_COMMAND
    ...
    MELAout << "Initial MC loop over " << nevents << " events in " << sample_tree.sampleIdentifier << " to determine sample normalization:" << endl;
    ...
    for (int ev=ev_start; ev<ev_end; ev++){
      ...
      if (event_wgt==0.f) continue;
      event_wgt *=  xsec * 1000.f * lumi / sum_wgts;
      ...
      if (do_ee_mumu==0 && !is_ee) continue;
      else if (do_ee_mumu==1 && !is_mumu) continue;
...
void makePlots(int do_ee_mumu, int procsel=-1, bool doOS=true, int doDR0p4=false, bool doRatio=false, TString strdate=""){
  gStyle->SetOptStat(0);

  if (strdate=="") strdate = HelperFunctions::todaysdate();

  TString const cinput_main = "output/LepEffFromMC/SkimTrees/" + strdate + (do_ee_mumu==0 ? "/Zee" : "/Zmumu");
  TString coutput_main = cinput_main + "/Plots" + (doOS? "/OS" : "/SS") + (!doDR0p4 ? "/DR0p3" : "/DR0p4") + (doRatio ? "/Ratios" : "/Nevents");
  gSystem->mkdir(coutput_main, true);
...
#undef BRANCH_COMMAND
  ...
  std::vector<TString> sampleLabelList;
  float lumi = SampleHelpers::getIntegratedLuminosity("2018B");
  sampleList.emplace_back("DY_2l_M_50_HT"); sampleLabelList.push_back("DY (HT-binned)");
  sampleList.emplace_back("TT2L2Nu"); sampleLabelList.push_back("t#bar{t}#rightarrow2l2#nu2b");
  sampleList.emplace_back("VV2L2X"); sampleLabelList.push_back("VV#rightarrow2l2X (X=#nu, q)");
  sampleList.emplace_back("WVLNu2X"); sampleLabelList.push_back("WV#rightarrowl#nu+2X (X=#nu, q)");
  sampleList.emplace_back("WZ3LNu"); sampleLabelList.push_back("WZ#rightarrow3l#nu");
  sampleList.emplace_back("QCD_HT"); sampleLabelList.push_back("QCD (HT-binned)");
  /*
  sampleList.emplace_back("ZZ2L2Nu");
  sampleList.emplace_back("ZZ2L2Q");
  sampleList.emplace_back("WW2L2Nu");
  sampleList.emplace_back("WWLNuQQ");
  sampleList.emplace_back("WZ3LNu");
  sampleList.emplace_back("WZ2L2Q");
  sampleList.emplace_back("WZL3Nu");
  sampleList.emplace_back("WZLNuQQ");
  */
  if (procsel>=0){
    if ((unsigned int) procsel>=sampleList.size()) return;

    std::vector<TString> sampleList_new;
    sampleList_new.push_back(sampleList.at(procsel));
    std::swap(sampleList, sampleList_new);

    std::vector<TString> sampleLabelList_new;
    sampleLabelList_new.push_back(sampleLabelList.at(procsel));
    std::swap(sampleLabelList, sampleLabelList_new);
  }
...
#undef BRANCH_COMMAND
  ...
  for (auto const& cutset:cutsets){
    ...
    MELAout << "\t- Begin cut set " << cuttitle << endl;
    ...
    if (procsel>=0){
      for (auto& hspec:hspecs){
        HelperFunctions::replaceString(hspec.name, "h1D", (const char*) Form("h1D_%s", sampleList.front().Data()));
        HelperFunctions::replaceString(hspec.title, strChannelLabel.Data(), (const char*) Form("%s (%s)", strChannelLabel.Data(), sampleLabelList.front().Data()));
      }
    }
    ...
    for (unsigned int isample=0; isample<nsamples; isample++){
      ...
      for (int ev=0; ev<tin->GetEntries(); ev++){
        ...
        if (do_ee_mumu==0){
          if (pass_cutbasedtight) defaultSelection.Fill(mass_ll, event_wgt);
        }
        else{
          if (pass_cutbasedtight && std::max(relpfiso04_l1, relpfiso04_l2)<0.15) defaultSelection.Fill(mass_ll, event_wgt);
        }
        auto it_hist = hlist.begin();
        if (do_ee_mumu==0){
          if (pass_fall17v2mvanoisowp90 && relpfiso_l1<0.2 && relpfiso_l2<0.2) (*it_hist)->Fill(mass_ll, event_wgt); it_hist++;
          ...
          */
          ...
          if (pass_cutbasedloose && relpfiso_l1<0.2 && relpfiso_l2<0.2) (*it_hist)->Fill(mass_ll, event_wgt); it_hist++;
          if (pass_cutbasedloose && relpfiso_l1<0.15 && relpfiso_l2<0.15) (*it_hist)->Fill(mass_ll, event_wgt); it_hist++;
          if (pass_cutbasedmedium && relpfiso_l1<0.2 && relpfiso_l2<0.2) (*it_hist)->Fill(mass_ll, event_wgt); it_hist++;
          if (pass_cutbasedmedium && relpfiso_l1<0.15 && relpfiso_l2<0.15) (*it_hist)->Fill(mass_ll, event_wgt); it_hist++;
          if (!doDR0p4){
            if (pass_cutbasedtight && relpfiso_l1<0.15 && relpfiso_l2<0.15) (*it_hist)->Fill(mass_ll, event_wgt); it_hist++;
          }
          if (pass_cutbasedtight && relpfiso04_l1<0.15 && relpfiso04_l2<0.15) (*it_hist)->Fill(mass_ll, event_wgt); it_hist++;
        }
      }
    ...
    double ymax = -1;
    {
      ...
      for (auto& hist:hlist){
        ...
        if (!doRatio){
          ...
          MELAout << "Integral[" << hist->GetName() << "](sb1, sb2, sb1+sb2, sr, sr/sb1+sb2) = ( " << int_sb1 << ", " << int_sb2 << ", " << int_sb1+int_sb2 << ", " << int_sr << ", " << int_sr/(int_sb1+int_sb2) << " )" << endl;
        }
      }
    }
    double yinf=ymin;
    ...
    for (auto const& hist:hlist) hist->GetYaxis()->SetRangeUser(ymin, ymax);


    TString canvasname = Form("%s_%s_%s", strChannel.Data(), "mll", cuttitle.Data());
    if (procsel>=0) HelperFunctions::replaceString(canvasname, strChannel.Data(), (const char*) Form("%s_%s", strChannel.Data(), sampleList.front().Data()));
    ...
    if (!doRatio){
      for (auto const& hist:hlist) hist->GetYaxis()->SetRangeUser(yinf*0.5, ysup*15.);
      canvas->Modified();
      canvas->Update();
      canvas->SetLogy();
      canvas->Modified();
...
void makePlots_pTll(int do_ee_mumu, bool doOS=true, bool doSB=false, bool doRatio=false, TString strdate=""){
  ...
  TString const cinput_main = "output/LepEffFromMC/SkimTrees/" + strdate + (do_ee_mumu==0 ? "/Zee" : "/Zmumu");
  TString coutput_main = cinput_main + "/Plots" + (doOS ? "/OS" : "/SS") + "/pTllComparisons" + (!doSB ? (do_ee_mumu==1 && !doOS ? "/SRandSideband" : "/SignalRegion") : "/Sidebands") + (doRatio ? "/Ratios" : "/Nevents");
  gSystem->mkdir(coutput_main, true);
  ...
  for (auto const& cutset:cutsets){
    ...
    for (unsigned int isample=0; isample<nsamples; isample++){
      ...
      for (int ev=0; ev<tin->GetEntries(); ev++){
        ...
        if (do_ee_mumu==0){
          if (pass_cutbasedtight) defaultSelection.Fill(pt_ll, event_wgt);
        }
        else{
          if (pass_cutbasedtight && max_relpfiso04<0.15) defaultSelection.Fill(pt_ll, event_wgt);
        }

        auto it_hist = hlist.begin();
        if (do_ee_mumu==0){
          if (pass_fall17v2mvanoisowp90 && max_relpfiso03<0.2) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_fall17v2mvanoisowp90 && max_relpfiso03<0.1) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_cutbasedloose) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_cutbasedmedium) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_cutbasedtight) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
        }
        else{
          if (pass_cutbasedloose && max_relpfiso03<0.2) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_cutbasedloose && max_relpfiso03<0.15) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_cutbasedmedium && max_relpfiso03<0.2) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_cutbasedmedium && max_relpfiso03<0.15) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_cutbasedtight && max_relpfiso03<0.15) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
          if (pass_cutbasedtight && max_relpfiso04<0.15) (*it_hist)->Fill(pt_ll, event_wgt); it_hist++;
        }
      }
...
void makePlots_pTl(int do_ee_mumu, bool doOS=true, bool doSB=false, bool doRatio=false, TString strdate=""){
  ...
  TString const cinput_main = "output/LepEffFromMC/SkimTrees/" + strdate + (do_ee_mumu==0 ? "/Zee" : "/Zmumu");
  TString coutput_main = cinput_main + "/Plots" + (doOS ? "/OS" : "/SS") + "/pTlComparisons" + (!doSB ? (do_ee_mumu==1 && !doOS ? "/SRandSideband" : "/SignalRegion") : "/Sidebands") + (doRatio ? "/Ratios" : "/Nevents");
  gSystem->mkdir(coutput_main, true);
  ...
  for (auto const& cutset:cutsets){
    ...
    for (unsigned int isample=0; isample<nsamples; isample++){
      ...
      for (int ev=0; ev<tin->GetEntries(); ev++){
        ...
        if (do_ee_mumu==0){
          if (pass_cutbasedtight){ defaultSelection.Fill(pt_l1, event_wgt); defaultSelection.Fill(pt_l2, event_wgt); }
        }
        else{
          if (pass_cutbasedtight && max_relpfiso04<0.15){ defaultSelection.Fill(pt_l1, event_wgt); defaultSelection.Fill(pt_l2, event_wgt); }
        }

        auto it_hist = hlist.begin();
        if (do_ee_mumu==0){
          if (pass_fall17v2mvanoisowp90 && max_relpfiso03<0.2){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_fall17v2mvanoisowp90 && max_relpfiso03<0.1){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_cutbasedloose){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_cutbasedmedium){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_cutbasedtight){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
        }
        else{
          if (pass_cutbasedloose && max_relpfiso03<0.2){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_cutbasedloose && max_relpfiso03<0.15){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_cutbasedmedium && max_relpfiso03<0.2){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_cutbasedmedium && max_relpfiso03<0.15){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_cutbasedtight && max_relpfiso03<0.15){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
          if (pass_cutbasedtight && max_relpfiso04<0.15){ (*it_hist)->Fill(pt_l1, event_wgt); (*it_hist)->Fill(pt_l2, event_wgt); }; it_hist++;
        }
      }
