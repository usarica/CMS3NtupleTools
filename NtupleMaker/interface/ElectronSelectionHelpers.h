...
namespace ElectronSelectionHelpers{
  ...
  float electronPFIsoComb(pat::Electron const& obj, int const& year, ElectronSelectionHelpers::IsolationType const& type, double const& rho, double const& fsr); // Absolute PF iso. value, uses rho instead of delta beta
  float electronMiniIsoComb(pat::Electron const& obj, int const& year, double const& rho, double const& fsr); // Absolute mini. iso. value
