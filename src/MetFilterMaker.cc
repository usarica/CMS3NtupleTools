...
void MetFilterMaker::produce( Event& iEvent, const edm::EventSetup& iSetup ) {
  ...
  auto_ptr <bool> filt_badMuons                      ( new bool(false) );
  auto_ptr <bool> filt_duplicateMuons                ( new bool(false) );
  auto_ptr <bool> filt_noBadMuons                    ( new bool(false) );
  ...
  *filt_hbheFilter                           = (idx_hbheNoise < 0) ? false : metFilterResultsH_->accept(idx_hbheNoise                       );
  ...
  iEvent.put( filt_badMuons                   , branchprefix_ + "badMuons"                   );
  iEvent.put( filt_duplicateMuons                   , branchprefix_ + "duplicateMuons"                   );
  iEvent.put( filt_noBadMuons                   , branchprefix_ + "noBadMuons"                   );
