...
//class EcalDeadCellEventFilter : public edm::EDFilter {
class EcalDeadCellEventFilter : public edm::EDProducer {
public:
  explicit EcalDeadCellEventFilter(const edm::ParameterSet&);
...
void EcalDeadCellEventFilter::loadEventInfoForFilter(const edm::Event &iEvent){
...
void EcalDeadCellEventFilter::loadEventInfo(const edm::Event& iEvent, const edm::EventSetup& iSetup){
   run = iEvent.id().run();
...
void EcalDeadCellEventFilter::loadEcalDigis(edm::Event& iEvent, const edm::EventSetup& iSetup){

  iEvent.getByLabel(tpDigiCollection_, pTPDigis);
  if ( !pTPDigis.isValid() ) { edm::LogWarning("EcalDeadCellEventFilter") << "Can't get the product " << tpDigiCollection_.instance()
                                             << " with label " << tpDigiCollection_.label(); return; }
}

void EcalDeadCellEventFilter::loadEcalRecHits(edm::Event& iEvent, const edm::EventSetup& iSetup){
...
EcalDeadCellEventFilter::EcalDeadCellEventFilter(const edm::ParameterSet& iConfig){
  ...
  //
  produces<bool> ( "ecalnoiseTPFilter" ).setBranchAlias("ecalnoise_TPFilter");
...
void EcalDeadCellEventFilter::envSet(const edm::EventSetup& iSetup) {
...
//bool EcalDeadCellEventFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup) {
void EcalDeadCellEventFilter::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  iEvent.put( pOut, "ecalnoiseTPFilter" );
...
//bool EcalDeadCellEventFilter::endRun(edm::Run &run, const edm::EventSetup& iSetup) {
void EcalDeadCellEventFilter::endRun(edm::Run &run, const edm::EventSetup& iSetup) {
  //return true;
  return;
}

int EcalDeadCellEventFilter::setEvtRecHitstatus(const double &tpValCut, const int &chnStatus, const int &towerTest){
...
int EcalDeadCellEventFilter::setEvtTPstatus(const double &tpValCut, const int &chnStatus){
...
int EcalDeadCellEventFilter::getChannelStatusMaps(){
...
DEFINE_FWK_MODULE(EcalDeadCellEventFilter);
