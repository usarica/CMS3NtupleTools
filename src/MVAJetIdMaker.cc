...
#include "CMS3/NtupleMaker/interface/MVAJetIdMaker.h"
...
void MVAJetIdMaker::produce(edm::Event& iEvent, const edm::EventSetup& iSetup){
    ...
    bool verbose = (ls == 147433 && evt == 44220807);
    ...
    for(int i0   = 0; i0 < (int) lUCJets.size(); i0++) {   // uncorrecte jets collection                                           
        ...
        for(int i1 = 0; i1 < (int) lCJets.size(); i1++) {   // corrected jets collection                                         
            const PFJet *tmp_pCJet = &(lCJets.at(i1));
            double deta = pUCJet->eta() - tmp_pCJet->eta();
            double dphi = acos(cos(pUCJet->phi() - tmp_pCJet->phi()));
        ...
        const PFJet *pCJet  = &(lCJets.at(best_index));
...
//		  if( !passPFLooseId(pUCJet)                                    ) continue;
        ...
        if (verbose && label_.find("full53x"))
            printf("jet pt, mva value: %4.2f, ", pUCJet->pt());
